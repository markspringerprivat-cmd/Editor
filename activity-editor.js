(() => {
  ...
  function initInteractiveVideoEditor() {
    ...
    function bindIvEvents() {
      app.querySelector('#videoUrl').oninput = e => { data.media = e.target.value; save(); window.clearTimeout(data._urlTimer); data._urlTimer = window.setTimeout(() => render(), 650); };
      app.querySelector('#loadVideo').onclick = () => { data.media = app.querySelector('#videoUrl').value.trim(); render(); };
      app.querySelector('#videoFile').onchange = e => { const file = e.target.files?.[0]; if (!file) return; if (file.size > 250 * 1024 * 1024) { alert('Die Videodatei ist sehr groß. Bitte nutze für den Test eine kleinere Datei oder eine URL.'); return; } const url = URL.createObjectURL(file); if (data._objectUrl) URL.revokeObjectURL(data._objectUrl); data.media = url; data._objectUrl = url; data._assetName = 'assets/' + file.name.replace(/[^a-zA-Z0-9._-]/g, '_'); mediaFileStore.set(data.id, file); render(); };
